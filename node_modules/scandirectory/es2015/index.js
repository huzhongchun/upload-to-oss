...
var readdir = require('readdir-cluster');
...
function scandirectory() {
	...
	if (opts.next == null) opts.next = function (err) {
		if (err) throw err;
	};
	var next = opts.next;
	...
	if (!opts.path) {
		if (opts.parentPath) {
			...
			var err = new Error('scandirectory: path is needed');
			return next(err);
		}
	}
	...
	if (opts.readFiles && next.length < 2) {
		var _err = new Error('scandirectory: readFiles is set but not enough completion callback arguments to receive the data');
		return next(_err);
	}
	...
	function iterator(fullPath, filename, stat) {
		// Prepare
		var relativePath = relative(opts.path, fullPath);
		if (next.length >= 2) {
		...
		if (stat.directory) {
			// Skip?
			var skip = opts.dirAction === false || opts.dirAction && opts.dirAction(fullPath, relativePath, filename, stat) === false;
		...
		// File
				...
				// Skip?
				var _skip = opts.fileAction === false || opts.fileAction && opts.fileAction(fullPath, relativePath, filename, stat) === false;
				if (_skip) return false;

				// Append
				if (opts.readFiles) {
					// Read file
					var data = readFileSync(fullPath);
		...
		if (err) {
			...
			(function () {
				...
				Object.keys(list).sort().forEach(function (relativePath) {
					var filename = files[relativePath];
					// root?
					if (relativePath === filename) {
						tree[relativePath] = trees[relativePath];
					} else {
						var parent = join(relativePath, '..');
